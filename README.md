<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,45:0F766E,100:0369A1&height=220&section=header&text=Vivek%20Painjane&fontSize=56&fontColor=FFFFFF&animation=fadeIn&fontAlignY=38&desc=Full-Stack%20Developer%20%7C%20AI%2FML%20Builder%20%7C%20Computer%20Engineering%20Student&descAlignY=58&descSize=17" width="100%" />

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=23&duration=2600&pause=900&color=38BDF8&center=true&vCenter=true&width=900&lines=Building+practical+software+products;Working+across+AI%2C+fintech%2C+taxtech+and+automation;Full-stack+engineering+with+a+p<div align="center">

# Vivek Painjane

### Product engineer building applied AI and financial software

Computer Engineering at PCCOE, Pune

[Portfolio](https://portfolio-azure-ten-62.vercel.app/) · [LinkedIn](https://www.linkedin.com/in/painjanevivek/) · [GitHub](https://github.com/painjanevivek) · [Email](mailto:painjanevivek@gmail.com)

</div>

---

## I build products around problems worth solving

I am a full-stack and AI engineer based in Pune. My work sits at the intersection of software, data, and product thinking, with a current focus on personal finance, creative intelligence, fraud analytics, and workflow automation.

I like finding the business hidden inside an inefficient process, then building the product end to end: shaping the workflow, designing the interface, developing the backend, securing the APIs, and deploying a system people can use.

Right now, I am building two main products:

- [MoneyKai](https://github.com/painjanevivek/MoneyKai), a personal finance app for managing everyday financial life
- [IroGuide](https://github.com/painjanevivek/IroGuide), an AI critique workspace that turns creative feedback into an iterative process

## Flagship products

### MoneyKai

**A personal finance system designed around the way people actually manage money.**

MoneyKai brings expenses, budgets, savings, shared costs, notes, analytics, billing, imports, and backup workflows into one product. It is being built as a real financial application, with authenticated data flows, privacy controls, and careful handling of sensitive operations.

| Product | Engineering |
|---|---|
| Expense, budget, savings, and shared expense workflows | Expo, React, TypeScript, Firebase, Firestore |
| Statement and Gmail imports | Authentication and backend-controlled sensitive flows |
| Analytics, notes, backup, and restore | Security headers, rate limits, and secret separation |
| AI-assisted financial review | Cross-platform product architecture |

[View the repository](https://github.com/painjanevivek/MoneyKai)

### IroGuide

**An AI design critique platform built for revision, not one-off feedback.**

IroGuide lets creators upload work, receive structured visual feedback, generate improvement plans, compare revisions, and keep a history of their progress. The platform pairs AI-assisted analysis with the product infrastructure needed for repeat use.

| Product | Engineering |
|---|---|
| Structured critique and improvement plans | Next.js, React, TypeScript, Firebase |
| Revision comparison and saved review history | Firestore and Firebase Storage |
| User profiles and account controls | Token verification and protected APIs |
| Persistent creator dashboard | Rate limiting, origin checks, log redaction, and SSRF protection |

[View the repository](https://github.com/painjanevivek/IroGuide)

## Selected work

| Project | What I built | Result |
|---|---|---|
| **MediGen** | An explainable generic medicine substitution model using linear algebra and classical ML | 90% Top-1 accuracy and 97% Top-3 accuracy, outperforming TF-IDF and Random Forest baselines |
| **Fraud transaction simulation** | A CTGAN and TVAE pipeline for generating minority-class banking transactions, evaluated with XGBoost | Fraud recall increased from 18% to 87%, F1 from 0.29 to 0.84, and AUC-ROC from 0.71 to 0.96 |
| **[HomeScape](https://github.com/painjanevivek/Homespace)** | A quiz-led housing recommendation product with a Python neural-network backend | Awarded a Government of India copyright |
| **[AI Global Intelligence](https://github.com/painjanevivek/AiGlobalIntel)** | An interactive GIS dashboard with choropleth risk maps, animated trade flows, and market markers | Combined geospatial visualization, React interaction, and modular Node APIs |

## Operating principles

| Principle | What it means in practice |
|---|---|
| Start with the workflow | Understand the user's decisions before choosing the stack |
| Build the whole loop | Connect the interface, data model, backend, security, and deployment |
| Measure the model | Use baselines and decision-relevant metrics instead of impressive demos |
| Treat trust as a feature | Keep sensitive operations on the backend and design for privacy from the start |
| Ship, observe, improve | Prefer working software and short feedback cycles over endless planning |

## Engineering toolkit

**Product development**

TypeScript, React, Next.js, Expo, Tailwind CSS, Vite

**Backend and data**

Node.js, Express, NestJS, FastAPI, Firebase, PostgreSQL, MySQL, MongoDB, SQLite, Prisma

**AI and analytics**

Python, Pandas, NumPy, Scikit-learn, PyTorch, XGBoost, SDV, CTGAN, TVAE, Plotly

**Infrastructure**

AWS, Docker, GitHub Actions, Vercel, Git, Firebase

## Current bets

- Making MoneyKai useful for recurring, messy, real-world finance workflows
- Developing IroGuide into a repeatable feedback system for creative work
- Building GST reconciliation modules through GstFlow
- Studying secure backend architecture, AI agents, and production SaaS systems

## Signals of execution

- Government of India copyright for the HomeScape housing recommendation system
- Research paper under review at the SSRG International Journal of Engineering Trends and Technology, Article ID ETT26MAY008
- Participant in the OpenAI Buildathon, Google Cloud Agentic AI Hackathon, and Codigo PCCOE
- First-place finishes in Valorant tournaments at SILVERA, CYBERKAVACH, and ANVESHAN

## Let us build something useful

I am open to product engineering, backend, and applied AI opportunities. I am especially interested in working with small teams that care about ownership, fast learning, and software that solves a specific problem well.

The best way to reach me is through [LinkedIn](https://www.linkedin.com/in/painjanevivek/) or [email](mailto:painjanevivek@gmail.com).

<div align="center">

<sub>Pune, India · Building in public at <a href="https://github.com/painjanevivek">github.com/painjanevivek</a></sub>

</div>
roduct+mindset;Clean+systems.+Useful+features.+Better+shipping." alt="Typing SVG" />

<br />

<img src="https://img.shields.io/badge/B.Tech-Computer%20Engineering-0F766E?style=flat-square&logo=academia&logoColor=white" />
<img src="https://img.shields.io/badge/PCCOE-Pune-0369A1?style=flat-square&logo=google-scholar&logoColor=white" />
<img src="https://img.shields.io/badge/Location-Pune%2C%20India-0F172A?style=flat-square&logo=googlemaps&logoColor=white" />

<br />
<br />

<a href="https://portfolio-azure-ten-62.vercel.app/">
  <img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=vercel&logoColor=38BDF8" />
</a>
<a href="https://www.linkedin.com/in/painjanevivek/">
  <img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=38BDF8" />
</a>
<a href="mailto:painjanevivek@gmail.com">
  <img src="https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=38BDF8" />
</a>
<a href="https://github.com/painjanevivek">
  <img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=38BDF8" />
</a>

<br />
<br />

<img src="https://komarev.com/ghpvc/?username=painjanevivek&style=flat-square&color=0891B2&label=Profile%20Views" />
<img src="https://img.shields.io/github/followers/painjanevivek?style=flat-square&color=0F766E&label=Followers&logo=github" />
<img src="https://img.shields.io/github/stars/painjanevivek?style=flat-square&color=0369A1&label=Stars&logo=github" />

</div>

---

## About

I am Vivek Painjane, a Computer Engineering student at PCCOE Pune. I build practical software products across full-stack development, AI/ML, fintech, healthcare, and business automation.

Most of my work starts from a real problem: tracking money better, reviewing creative work with AI, simulating fraud transactions, or turning raw information into dashboards people can actually use. I care about simple product flows, clean backend design, secure APIs, and shipping things that work outside a demo.

I work mainly with Python, TypeScript, React, Node.js, Firebase, cloud tools, and applied machine learning libraries. I am currently learning deeper backend architecture, API security, AI agents, and production SaaS patterns.

**Open to**

| Area | Interest |
|---|---|
| Software Engineering | Full-stack, backend, product engineering |
| AI/ML Engineering | Applied ML, GenAI workflows, fraud analytics, recommendation systems |
| Open Source | Developer tools, automation, practical AI systems |
| Internships | Software engineering, AI engineering, cloud/backend engineering |

---

## Tech Stack

### Languages

<p align="center">
  <img src="https://skillicons.dev/icons?i=c,cpp,python,java,js,ts" />
</p>

### Frontend

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,react,nextjs,vite,tailwind" />
</p>

### Mobile

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,expo" />
</p>

### Backend & Databases

<p align="center">
  <img src="https://skillicons.dev/icons?i=nodejs,express,nestjs,fastapi,firebase,sqlite,mysql,mongodb,postgres,prisma" />
</p>

### Cloud, DevOps & Tooling

<p align="center">
  <img src="https://skillicons.dev/icons?i=aws,docker,git,github,githubactions,vercel,vscode,figma" />
</p>

### AI / ML & Data

<p align="center">
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white" />
  <img src="https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white" />
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/Plotly-3F4F75?style=flat-square&logo=plotly&logoColor=white" />
  <img src="https://img.shields.io/badge/XGBoost-0F766E?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Prompt%20Engineering-0369A1?style=flat-square&logo=openai&logoColor=white" />
</p>

---

## AI / ML expertise

| Domain | Proficiency | Details |
|---|---:|---|
| Applied Machine Learning | Advanced | Built ML systems for medicine substitution, housing recommendations, and fraud analytics |
| Generative AI | Advanced | Worked with CTGAN, TVAE, SDV, and XGBoost for synthetic fraud transaction simulation |
| AI Product Engineering | Advanced | Built AI-assisted feedback, review history, improvement plans, and user dashboards |
| Data Science | Intermediate | Uses Pandas, NumPy, Scikit-learn, Plotly, metrics, baselines, and model evaluation |
| Recommendation Systems | Intermediate | Built quiz-based recommendation workflows with a Python ML backend |
| API Security | Intermediate | Applies auth checks, rate limits, secure headers, log redaction, and backend-first handling |

---

## Featured projects

<details open>
<summary><b>MoneyKai - Personal Finance & Budget Management App</b></summary>

<br />

A personal finance app for expense tracking, budgeting, savings, shared expenses, notes, analytics, backup and restore, billing, AI review, Gmail imports, statement imports, and privacy-focused finance workflows.

| Category | Details |
|---|---|
| Stack | Expo, React, TypeScript, Firebase, Firestore |
| Scale | Cross-platform finance app with authenticated user data flows |
| Performance | Responsive finance tracking, analytics, imports, and backup workflows |
| Security | Firebase auth, backend-first sensitive flows, security headers, rate limits, secret separation |
| Impact | Helps users manage money, budgets, shared expenses, savings, and financial records |
| Repository | [github.com/painjanevivek/MoneyKai](https://github.com/painjanevivek/MoneyKai) |

MoneyKai is built like a real product, with user accounts, privacy pages, security checks, and workflows for daily personal finance use.

</details>

<details>
<summary><b>IroGuide - AI Design Critique Platform</b></summary>

<br />

A full-stack AI platform where users upload creative work, receive structured visual feedback, generate improvement plans, compare revisions, and keep review history.

| Category | Details |
|---|---|
| Stack | Next.js, React, TypeScript, Firebase, Firestore, Firebase Storage |
| Scale | Deployed full-stack platform with dashboards and persistent review history |
| Performance | Responsive public pages and optimized upload/review flows |
| Security | Token verification, rate limiting, secure headers, origin checks, log redaction, SSRF protection |
| Impact | Turns creative work into specific feedback and revision plans |
| Repository | [github.com/painjanevivek/IroGuide](https://github.com/painjanevivek/IroGuide) |

IroGuide combines AI feedback with ordinary product details that matter: auth, saved history, profile controls, account deletion, and protected APIs.

</details>

<details>
<summary><b>MediGen - Generic Medicine Substitution</b></summary>

<br />

An explainable model for generic medicine substitution using linear algebra and classical ML evaluation.

| Category | Details |
|---|---|
| Stack | Python, Linear Algebra, Scikit-learn |
| Scale | Personal ML project |
| Performance | 90% Top-1 accuracy and 97% Top-3 accuracy |
| Security | Explainable rule-based approach for healthcare-oriented decision support |
| Impact | Outperformed TF-IDF and Random Forest baselines |
| Repository | [github.com/painjanevivek](https://github.com/painjanevivek) |

MediGen keeps the model logic understandable, which matters in healthcare-style recommendation problems.

</details>

<details>
<summary><b>AI Global Intelligence Platform</b></summary>

<br />

An interactive GIS dashboard with choropleth risk maps, animated trade-flow arcs, market markers, command-bar navigation, and a modular React/Node architecture.

| Category | Details |
|---|---|
| Stack | React, TypeScript, MapLibre GL JS, Node.js, Express.js |
| Scale | Academic full-stack development lab mini project |
| Performance | Interactive geospatial dashboard with animated map layers |
| Security | Modular API structure and controlled dashboard workflows |
| Impact | Turns global intelligence data into an explorable map interface |
| Repository | [github.com/painjanevivek/AiGlobalIntel](https://github.com/painjanevivek/AiGlobalIntel) |

This project brings together frontend interaction work, GIS visualization, backend APIs, and data presentation.

</details>

<details>
<summary><b>Generative AI for Fraud Transaction Simulation in Banking</b></summary>

<br />

A synthetic fraud data pipeline for imbalanced banking datasets using generative AI and supervised fraud detection.

| Category | Details |
|---|---|
| Stack | CTGAN, TVAE, XGBoost, Python, SDV |
| Scale | Generative AI applications project |
| Performance | Fraud Recall improved from 18% to 87%, F1 from 0.29 to 0.84, AUC-ROC from 0.71 to 0.96 |
| Security | Uses synthetic data for fraud analytics experiments |
| Impact | Improves fraud detection performance on imbalanced financial datasets |
| Repository | [github.com/painjanevivek](https://github.com/painjanevivek) |

The project tests whether generated fraud examples can make downstream detection models more useful.

</details>

<details>
<summary><b>HomeScape - ML-Based Housing Recommendation System</b></summary>

<br />

An ML-powered housing recommendation system using quiz-based user inputs, a responsive frontend, and a Python neural-network backend.

| Category | Details |
|---|---|
| Stack | Python, HTML, CSS, JavaScript, Neural Networks |
| Scale | Personal ML project |
| Performance | Personalized recommendations from structured user preference inputs |
| Security | Controlled input-driven recommendation workflow |
| Impact | Awarded Government of India Copyright |
| Repository | [github.com/painjanevivek/Homespace](https://github.com/painjanevivek/Homespace) |

HomeScape turns user preferences into housing suggestions through a simple ML-backed flow.

</details>

## Achievements

| Recognition | Details |
|---|---|
| Government of India Copyright | HomeScape ML-Based Housing Recommendation System |
| Research Paper Under Review | SSRG International Journal of Engineering Trends and Technology, Article ID: ETT26MAY008 |
| GenAI Fraud Analytics | Improved Fraud Recall from 18% to 87%, F1 from 0.29 to 0.84, AUC-ROC from 0.71 to 0.96 |
| Hackathon Participation | OpenAI Buildathon, Google Cloud Agentic AI Hackathon, Codigo PCCOE |
| Competitive Gaming Wins | 1st Prize in Valorant tournaments at SILVERA, CYBERKAVACH, and ANVESHAN |

---

## Certifications

### AWS

<p>
  <img src="https://img.shields.io/badge/AWS%20For%20Everyone-Udemy-0F766E?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/39%20Hours-April%202026-0F172A?style=for-the-badge" />
</p>

---

## Contribution activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=painjanevivek&bg_color=0D1117&color=C9D1D9&line=22D3EE&point=38BDF8&area=true&hide_border=true&custom_title=Contribution%20Activity" width="100%" alt="Contribution activity graph" />

</div>

---

## Current focus

```yaml
Learning:
  - Backend architecture
  - API security
  - Applied AI and ML pipelines
  - Cloud deployment and DevOps

Building:
  - MoneyKai finance workflows
  - IroGuide AI review flows
  - GstFlow reconciliation modules
  - Fraud simulation experiments

Exploring:
  - AI agents
  - SaaS product patterns
  - Secure full-stack systems
  - Data products

Open To:
  - Software engineering internships
  - AI/ML engineering work
  - Open-source collaboration
  - Product-focused engineering teams
```

---

## Connect

<div align="center">

<a href="mailto:painjanevivek@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-0D1117?style=for-the-badge&logo=gmail&logoColor=38BDF8" />
</a>
<a href="https://www.linkedin.com/in/painjanevivek/">
  <img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=38BDF8" />
</a>
<a href="https://github.com/painjanevivek">
  <img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=38BDF8" />
</a>
<a href="https://portfolio-azure-ten-62.vercel.app/">
  <img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=vercel&logoColor=38BDF8" />
</a>

</div>
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/painjanevivek/painjanevivek/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/painjanevivek/painjanevivek/output/github-contribution-grid-snake.svg" />
  <img alt="GitHub contribution snake" src="https://raw.githubusercontent.com/painjanevivek/painjanevivek/output/github-contribution-grid-snake.svg" width="100%" />
</picture>
---

<div align="center">

Building software that is useful, secure, and easy to keep improving.

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0369A1,45:0F766E,100:0D1117&height=130&section=footer" width="100%" />

</div>
